...
const schema=z.object({schemaVersion:z.literal(4),revision:z.number().int().nonnegative(),updatedAt:z.string(),profile:z.object({name:z.string(),onboarded:z.boolean(),allergies:z.array(z.string()),conditions:z.array(z.string()),dietaryPreference:z.string()}).passthrough(),onboarding:z.object({step:z.number().int().min(0).max(7),complete:z.boolean()}),observations:z.array(observationSchema),plans:z.array(z.any()),preferenceEvents:z.array(z.any()),sessions:z.array(z.any()),occurrences:z.array(z.any()),diary:z.array(z.any()),settings:z.object({theme:z.string(),reduceTransparency:z.boolean()}).passthrough(),tombstones:z.record(z.string()),conflicts:z.array(z.any())}).passthrough();
